...
function isStaleClient(cached) {
  ...
  if (typeof cached.user_follows?.findMany !== "function") {
    return true;
  }
  if (typeof cached.article_reactions?.findMany !== "function") {
    return true;
  }
  if (typeof cached.favorite_folders?.findMany !== "function") {
    return true;
  }
  if (typeof cached.browse_history?.findMany !== "function") {
    return true;
  }
  if (typeof cached.chat_conversations?.findMany !== "function") {
    return true;
  }
  if (typeof cached.chat_conversation_reads?.findMany !== "function") {
    return true;
  }
  if (typeof cached.user_blocks?.findMany !== "function") {
    return true;
  }
  return false;
}
...
export async function readyDb() {
  if (!readyPromise) {
    readyPromise = (async () => {
      ...
      const { ensureAuthorLinks } = await import("@/lib/backfillAuthors");
      await ensureSeeded();
      await ensureAuthorLinks();
    })();
  }
  await readyPromise;
