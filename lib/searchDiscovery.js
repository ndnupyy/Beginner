...
function extractKeywordsFromArticle(article) {
  ...
  if (article.category?.trim()) {
    keywords.add(article.category.trim());
  }
  ...
  return [...keywords].filter((item) => !STOP_WORDS.has(item.toLowerCase()));
}
...
function getCollaborativeScore(keyword, seedArticles, allArticles) {
  ...
  seedArticles.forEach((sourceArticle) => {
    const sourceTags = new Set(sourceArticle.tags || []);
    ...
    allArticles.forEach((candidate) => {
      ...
      if (sharedTags.length === 0 && !sameCategory) return;
      if (!articleMatchesKeyword(keyword, candidate)) return;

      score += sharedTags.length * 2 + (sameCategory ? 3 : 0);
    });
  });
